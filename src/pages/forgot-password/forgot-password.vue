  ...
  <view class="container">
    <view class="reset-box">
      ...
      <view class="form-box">
        ...
        <view 
          class="reset-btn" 
          :class="{ disabled: isResetting || !isFormValid }" 
          @tap="resetPassword"
        >
      ...
      <view class="login-link">
        <text>记起密码了？</text>
        <text class="login-text" @tap="goToLogin">返回登录</text>
      </view>
    </view>
  </view>
</template>
...
const isPhoneValid = computed(() => {
  return /^1[3-9]\d{9}$/.test(resetForm.phone);
});
...
const sendCode = async () => {
  ...
  try {
    await app.globalData.api.auth.sendSmsCode(resetForm.phone, 'reset');
...
const resetPassword = async () => {
  ...
  try {
    // 调用重置密码API
    await app.globalData.api.auth.resetPassword(
      resetForm.phone,
      resetForm.code,
      resetForm.password,
    ...
    errors.general = error.message || '重置密码失败，请检查输入信息';
